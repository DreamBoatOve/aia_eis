...
if rank == 0:
    ...
    linear_svm_para_list, poly_svm_para_list, rbf_svm_para_list = svm_res_checker(folder='txt_res')
    para_list = linear_svm_para_list
    # para_list = linear_svm_para_list + poly_svm_para_list + rbf_svm_para_list
    # linear/Poly/Rbf各挑一种尝试运行
    # para_list = para_list[:15] + para_list[10000:10015] + para_list[-15:]
    ...
    for para in rec_para_list:
        ...
        multi_svm.create_svm_classifer(svm_model_pickle_name)
        sample_label_prob_dict_list = multi_svm.classify(svm_model_pickle_name)

        acc = cal_accuracy(sample_label_prob_dict_list, vali_label_list)
        kappa = cal_kappa(sample_label_prob_dict_list, vali_label_list)

        res_name = get_date_prefix()+'svm_mpi.txt'
...
Plan
    ...
    2-Run the file
        Use 'cmd' in the folder containing this file, then type the following command:
            mpiexec -n 16 python svm_main_mpi_0.py
"""