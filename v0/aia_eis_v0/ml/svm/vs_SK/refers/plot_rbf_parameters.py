...
to the score values of the best performing ovo_models so as to make it easy to tell
...
For intermediate values, we can see on the second plot that good ovo_models can
be found on a diagonal of ``C`` and ``gamma``. Smooth ovo_models (lower ``gamma``
...
performing ovo_models.

Finally one can also observe that for some intermediate values of ``gamma`` we
get equally performing ovo_models when ``C`` becomes very large: it is not
...
value of ``C`` so as to favor ovo_models that use less memory and that are faster
to predict.
