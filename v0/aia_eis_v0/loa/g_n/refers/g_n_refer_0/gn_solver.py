...
class GNSolver:
    ...
    def _calculate_pseudoinverse(x: np.ndarray) -> np.ndarray:
        """
        Moore-Penrose inverse. 摩尔－彭若斯广义逆
            https://www.geeksforgeeks.org/moore-penrose-pseudoinverse-mathematics/
            Moore – Penrose pseudoinverse is computed as A+ = (A^T @ A)^{-1} @ A^T
        线性代数：A转置乘以A可逆
            A^T @ A ==> AA是可逆矩阵
        """
