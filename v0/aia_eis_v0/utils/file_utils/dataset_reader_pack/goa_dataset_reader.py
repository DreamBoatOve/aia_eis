...
def normed_lai_eis(folder, sample_area = 1.01 * 1e-6):
    ...
    normed_eis_dict = {}
    for k, v in raw_eis_dict.items():
        normed_eis_dict[k] = {'fre': v['fre'], 'z' : [z * sample_area for z in v['z']]}

    fn_name = get_date_prefix() + 'goa_lai_normed_dataset_pickle.file'
    with open(fn_name, 'wb') as file:
        pickle.dump(normed_eis_dict, file)
