...
def detect(eis_source, pointNum=3, criteria_type='absRealAndAbsImag',
    ...
    while (pointCount < pointNum) and (chiSquare > chiSquareLimit):
        ...
        # bode_Phase(fre_arr=tmp_eis.fre_arr, z_arr_list=[z_arr, z_SG_arr], fig_title='Comparison', label_list=['Raw', 'SG'])
        # ------ Plot the smoothed results ------
        ...
        # -------- Plot the |▲|Z|| vs. Fre --------
        # p2u = plot_2D_util(x_list=np.log10(tmp_eis.fre_arr).tolist(), y_list=d_phase_arr.tolist())
        # p2u.single_dot_plot(x_label='log10(Fre)', y_label='|▲Phase| = | (Phase - Phase_SG) / Phase |', line_label_str='|▲Phase| vs. log10(Fre)')
        # -------- Plot the |▲|Z|| vs. Fre --------
...
# -------------------------------------- One Outlier --------------------------------------
# detect(eis_source = '2021_09_15_R(RC)(RW)_HF_M_ecm.pkl', pointNum = 1,
       # ----------------
       # criteria_type = 'absRe',
       # criteria_type = 'absIm',
...
# -------------------------------------- Two Outlier --------------------------------------
# detect(eis_source = '2021_10_13_R(RC)(RW)_HLF_MM_ecm.pkl', pointNum = 2,
       # ----------------
       # criteria_type = 'absRe',
       # criteria_type = 'absIm',
       # criteria_type = 'absRealAndAbsImag',
...
# -------------------------------------- Three Outlier --------------------------------------
# detect(eis_source = '2021_10_13_R(RC)(RW)_MMH_ecm.pkl', pointNum = 3,
       # ----------------
       # criteria_type = 'absRe',
       # criteria_type = 'absIm',
       # criteria_type = 'absRealAndAbsImag',
