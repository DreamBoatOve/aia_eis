...
def detect(eis_source, vogitAddC, pointNum=3, chiSquareLimit=1e-5, printFlag=True):
    ...
    while (pointCount < pointNum) and (chiSquare > chiSquareLimit):
        ...
        # |▲|Z|| = | (|Z| - |Zsg|) / |Z| |
        abs_d_absZ_arr = np.abs([(absZ - absZ_SG) / absZ for absZ, absZ_SG in zip(absZ_arr, absZ_SG_arr)])
        ...
        chiSquare_list = []
        if printFlag:
            print('------------- Index and Difference and chiSquare For Bd-|▲Z| --------------------')
        ...
        if printFlag:
            ...
            print('------------- ∑|ε_beforeDeletion| - ∑|ε_afterDeletion| Order (Big->Small) + ChiSquare Order (Small->Big) --------------------')
            print('∑|ε_beforeDeletion| - ∑|ε_afterDeletion|, sorted_dif_list.index(dif), chiS, sorted_chiSquare_list.index(chiS)')
