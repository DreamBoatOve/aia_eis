...
if rank == 0:
    ...
    linear_svm_para_list, poly_svm_para_list, rbf_svm_para_list = svm_res_checker(folder='ovr_txt_res')
    # NH55 focus on Linear + Poly + RBF training
    ...
    for para in rec_para_list:
        ...
        svm_model_pickle_path = 'ovr_models/' + svm_model_pickle_name
        multi_svm.create_svm_ovr_classifer(svm_model_pickle_path)
        sample_label_prob_dict_list = multi_svm.classify_ovr(svm_model_pickle_path)

        acc = cal_accuracy(sample_label_prob_dict_list, vali_label_list)
        kappa = cal_kappa(sample_label_prob_dict_list, vali_label_list)

        res_name = get_date_prefix() + 'svm_ovr_mpi.txt'
        res_path = 'ovr_txt_res/' + res_name
...
Plan
    ...
    2-Run the file
        Use 'cmd' in the folder containing this file, then type the following command:
            mpiexec -n 16 python svm_ovr_main_mpi_desktop.py
"""